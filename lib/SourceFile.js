...
SourceFile.prototype = {
    ...
    address: function() {
        ...
        if (this.relativeJumpTargets.length) {
            ...
            this.relativeJumpTargets.forEach(function(instr) {
                ...
                if (!target) {
                    that.addressError(
                        'Invalid jump offset, point at the address of a instruction',
                        instr.index
                    );
    ...
    checkForDefinition: function(entryType, name, index) {
        ...
        var existing = this.compiler.resolveName(name, this);
        ...
        // TODO check if the name would shadow a macro
    ...
    // Errors -----------------------------------------------------------------
    referenceError: function(msg, index) {
    ...
    addressError: function(msg, index) {
        var s = this.getLineSource(index),
    ...
    argumentError: function(msg, index) {
        var s = this.getLineSource(index),
    ...
    declarationError: function(msg, index, existing) {
        var s = this.getLineSource(index),
            message = 'ParseError: Redeclartion of ' + msg + ', first declared in ' + existing.file.getPath(existing.index);
    ...
    sectionMissingError: function(type, index) {
        var s = this.getLineSource(index),
            message = 'AddressError: Missing SECTION directive before ' + type;
    ...
    fileError: function(err, msg, filePath, index) {
        ...
        var s = this.getLineSource(index),
            message = 'FileError: Unable to ' + msg + ' "' + filePath + '", ';

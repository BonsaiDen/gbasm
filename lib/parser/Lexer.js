...
Lexer.prototype = {
    ...
    parseOffset: function(buffer, index, sign) {
        ...
        if (sign === 45) {
            ...
            this.file.parseError(String.fromCharCode(sign), 'valid direction specifier (- or +) for offset', index + 1);
        }
    ...
    parseName: function(buffer, index, ch) {
        ...
        if (ch === 58) {
            if (!this.name(name, true, index)) {
                this.file.parseError('invalid label name ' + name, null, at);
        ...
        while(ch !== delimiter) {
            ...
            if (ch === 92) {
                ...
                switch(ch) {
                    ...
                    default:
                        // TODO only put index onto token and use that to figure out the line / col later on
                        this.file.parseError('invalid escape sequence', null, index);
                        break;
                }
    ...
    token: function(type, value, index) {
        ...
        if (this.lastToken && this.lastToken.type === 'OFFSET_SIGN') {
            ...
            if (type !== 'NUMBER') {
                this.file.parseError(type, 'a number after @' + this.lastToken.value, index);
    ...
    expr: function(token, next) {
        ...
        if (isExpression(token, next)) {
            ...
            switch(token.type) {
                ...
                case 'RPAREN':
                    ...
                    if (this.parenLevel < 0) {
                        this.file.parseError('closing )', null, token.index);
                    }
                    break;
            ...
            if (this.expression.length) {
                ...
                if (this.parenLevel !== 0) {
                    this.file.parseError(next.type, 'closing )', next.index);
                ...
                // Parse the final expression
                    ...
                    if (this.expression[this.expression.length - 1].type === 'OPERATOR') {
                        this.file.parseError('unbound operator', 'expression value', token.index);
                ...
                // In case of a single token expression check if it is actually valid
                    ...
                    this.file.parseError(token.type, 'expression', token.index);
                }
    ...
    name: function(value, isLabel, index) {
        ...
        switch(value) {
            ...
            case 'stop':
            case 'swap':
                return isLabel ? false : this.token('INSTRUCTION', value, index);

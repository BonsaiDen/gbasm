...
Lexer.prototype = {
    ...
    expr: function(token, next) {
        ...
        if (isExpression(token, next, this.parenDepth)) {
            ...
            if (this.expression.length) {
                ...
                // If so parse them as a binary expression
                // using the recursive descent parser
                var binaryExpression = null;
                if (this.expression.length > 1) {
                    binaryExpression = new Expression(this, this.expression);
                }

                // Did we parse something and does it have more than one token?
                if (binaryExpression !== null && binaryExpression.length > 1) {

                    // Remove the consumed expression tokens from the token stream
                    this.tokens.length = this.tokens.length - binaryExpression.length;

                    // Insert the expression token
                    // TODO push actual binary expression

...
Parser.prototype = {
    ...
    parse: function() {
        ...
        while((this._token = this.stream.next())) {
            ...
            switch(this._token.type) {
                ...
                case 'NAME':
                    this.parseName(this.stream, this._token.value);
                    break;
                ...
                case 'LABEL_LOCAL_DEF':
                    ...
                    this.file.label(this._token.value, this._label);
                    break;
    ...
    parseName: function(s, name) {
        ...
        if (s.peak('EQU')) {
            ...
            if (value) {
                this.file.constant(name, value, false);
            }
            ...
            if (value) {
                this.file.constant(name, value, true);
            }
    ...
    parseLabel: function(s, name) {
        ...
        if (s.peak('DS')) {
            s.expect('DS');
            this.file.variable(name, s.expect('NUMBER'));

        // label: DB
        } else if (s.peak('DB')) {
            s.expect('DB');
            this.file.variable(name, 1);

        // label: DW
        } else if (s.peak('DW')) {
            s.expect('DW');
            this.file.variable(name, 2);

        // label:
        } else {
            this._label = this.file.label(name, null);
        }
    ...
    parseMacro: function(s, token) {
        ...
        switch(token.value) {
            ...
            case 'SECTION':
                name = s.expect('STRING').value;
                s.expect(',');
                segment = s.expect('NAME');
                s.expect('[');
                offset = s.expect('NUMBER');
                s.expect(']');
                this.file.section(name, segment, offset);
                break;

            // DS $ff
            case 'DS':
                this.file.data(s.expect('NUMBER'));
                break;

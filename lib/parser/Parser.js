...
Parser.prototype = {
        ...
        while((this._token = this.stream.next())) {
            ...
            switch(this._token.type) {
                ...
                case 'LABEL_GLOBAL_DEF':
                    this.parseLabel(this.stream, this._token.value);
                    break;

                case 'LABEL_LOCAL_DEF':
                    if (!this._label) {
                        this.file.parseError('local label', null, this._token.line, this._token.col);
                    }
                    this.file.labels.push(new Label(this._token.value, this.file.offset, this._label));
                    break;

                case 'MACRO':
                    this.parseMacro(this.stream, this._token.value);
                    break;
    ...
    parseLabel: function(s, name) {
        ...
        if (s.peak('DS')) {
            ...
            this.file.variables.push(new Variable(name, this.file.offset, size));
            this.file.offset += size;

        // label: DB
        } else if (s.peak('DB')) {
            s.expect('DB');
            this.file.variables.push(new Variable(name, this.file.offset, 1));
            this.file.offset++;

        // label: DW
        } else if (s.peak('DW')) {
            s.expect('DW');
            this.file.variables.push(new Variable(name, this.file.offset, 2));
            this.file.offset += 2;

        // label:
        } else {
            this._label = new Label(name, this.file.offset, null);
            this.file.labels.push(this._label);
        }

    },

    parseMacro: function(s, keyword) {
        ...
        switch(keyword) {
            ...
            case 'SECTION':
                ...
                s.expect(']');
                this.file.sections.push(new Section(name, segment, offset));
                this.file.offset = offset;
                break;

            // DS $ff
            case 'DS':
                this.file.offset += s.expect('NUMBER');
                break;
        ...
        switch(mnemonic) {
            ...
            case 'res':
                ...
                if (s.peak('[')) {
                    ...
                    s.expect(']');
                    if (mnemonic === 'bit') {
                        this.instruction(12, [0xCB, 0x46 + arg]);

                    } else if (mnemonic === 'set') {
                        this.instruction(16, [0xCB, 0xC6 + arg]);

                    } else if (mnemonic === 'res') {
                        this.instruction(16, [0xCB, 0x86 + arg]);
                    }
            ...
            case 'jr':
                ...
                if (s.peak('FLAG')) {
                    ...
                    switch(flag) {
                        case 'c':
                            this.instruction(12, [0x38], arg, true);
                            break;

                        case 'nc':
                            this.instruction(12, [0x30], arg, true);
                            break;

                        case 'z':
                            this.instruction(12, [0x28], arg, true);
                            break;

                        case 'nz':
                            this.instruction(12, [0x20], arg, true);
                            break;
                    }
                ...
                // jr label
                    ...
                    arg = s.expect('NUMBER_16BIT');
                    this.instruction(12, [0x18], arg, true);
                }
                break;
    ...
    instruction: function(cycles, code, arg, isByte, isSigned) {
        ...
        this.file.instructions.push(instr);

        this.file.offset += instr.size;
        this.file.size += instr.size;

...
Parser.prototype = {
        ...
        while((this._token = this.stream.next())) {
            ...
            switch(this._token.type) {
                ...
                case 'LABEL_LOCAL_DEF':
                    ...
                    this.file.addLabel(this._token.value, this._label, this._token.line, this._token.col);
                    break;
    ...
    parseName: function(s, name) {
        ...
        if (s.peak('EQU')) {
            ...
            if (value !== null) {
                this.file.addConstant(name.value, value, false, name.line, name.col);
            }
            ...
            if (value !== null) {
                this.file.addConstant(name.value, value, true, name.line, name.col);
            }

        } else {
            this.file.parseError(name.type, null, name.line, name.col);
        }
    ...
    parseLabel: function(s, label) {
        ...
        if (s.peak('DS')) {
            ...
            this.file.addVariable(label.value, size, label.line, label.col);

        // label: DB
        } else if (s.peak('DB')) {
            s.expect('DB');
            this.file.addVariable(label.value, 1, label.line, label.col);

        // label: DW
        } else if (s.peak('DW')) {
            s.expect('DW');
            this.file.addVariable(label.value, 2, label.line, label.col);

        // label:
        } else {
            this._label = this.file.addLabel(label.value, null, label.line, label.col);
        }
    ...
    parseDirective: function(s, token) {
        ...
        switch(token.value) {
            ...
            case 'INCBIN':
                name = s.expect('STRING');
                this.file.addBinaryInclude(name, name.line, name.col);
                break;
            ...
            case 'DS':
                ...
                if (s.peak('EXPRESSION')) {
                    value = s.expect('EXPRESSION');
                    this.file.addDataBlock([value], true, size);
                    ...
                    this.file.addDataBlock([], true, size);
                }
                break;
                ...
                if (s.peak('STRING')) {
                    while((value = s.expect('STRING'))) {
                        ...
                        values.push.apply(values, string.map(function(c, i) {
                            return new Lexer.Token('NUMBER', c.charCodeAt(0), value.line, value.col + i);
                        }));
                        ...
                        if (s.peak(',')) {
                            ...
                            this.file.addDataBlock(values, true, null);
                            values = [];
                            break;
                        }
            ...
            case 'DW':
                while((value = s.expect('NUMBER_16BIT'))) {
                    ...
                    if (s.peak(',')) {
                        ...
                        this.file.addDataBlock(values, false, null);
                        values = [];
                        break;
                    }

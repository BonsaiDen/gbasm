...
Parser.prototype = {
    ...
    parseMacro: function(s, keyword) {
        ...
        switch(keyword) {
            case 'INCLUDE':
                arg = s.expect('STRING');
                this.file.include(arg.value, arg.line, arg.col);
                break;
        }
    ...
    instruction: function(cycles, bytes, value, singleByteValue, signedValue) {

        var size = bytes.length + (value ? (singleByteValue ? 1 : 2) : 0);

        this.file.instructions.push([
            this._mnemonic, this.file.offset, size, cycles, bytes,
            value || null, !!singleByteValue, !!signedValue,
        ]);

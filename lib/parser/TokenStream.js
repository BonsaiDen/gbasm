...
TokenStream.prototype = {
    ...
    expect: function(type) {
        ...
        if (!this.token || type === 'EOF') {
            this.file.parseError('end of input', type, index);

        } else if (this.is(type, this.token)) {
            return this.value(type, this.next());

        } else {
            var info = this.value(this.token.type, this.token);
            this.file.parseError(typeof info === 'object' ? info.type : info, type, index);
        }
    ...
    is: function(type, token) {
        ...
        switch(type) {
            ...
            case 'BANK':
                return token.type === 'DIRECTIVE' && token.value === 'BANK';

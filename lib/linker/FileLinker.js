...
var FileLinker = {
    ...
    init: function(file) {
        ...
        file.sections.forEach(function(section) {
            ...
            var stack = 0;
            while(section.macros.length) {

                // Shallow clone the current macros before the expansions creates more
                var macros = section.macros.slice();
                ...
                for(var i = 0, l = macros.length; i < l; i++) {
                    ...
                    var expr = macros[i],
                        macro = Linker.resolveMacro(expr, section.file, 0, []);
                    ...
                    if (macro.isBuiltin) {
                        ...
                        // TODO do the actual expansion!
                        // TODO do this iteratively as long as we find new macro calls
                        // TODO when we expand we need to splice stuff into
                        // the existing data set of the file
                        macro.callee.expand(section.file, macro.args);
                    }

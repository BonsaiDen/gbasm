...
const Generator = {
    ...
    generateRom(files) {
        ...
        let buffer = Generator.getRomBuffer(files);
        files.forEach((file) => {
            Generator.generateFile(file, buffer);
        });

        // Pad if ROM size in header is bigger than generated buffer
        const rom = Generator.parseRom(buffer);
        if (rom.rom.size > buffer.length) {
            buffer = Generator.getPaddedRomBuffer(buffer, rom.rom.size);
    ...
    generateFile(file, buffer) {
        ...
        file.instructions.forEach((instr) => {

            let index = instr.offset;
            for(let i = 0; i < instr.raw.length; i++) {
        ...
        file.binaryIncludes.forEach((binary) => {
    ...
    // Size Calculation -------------------------------------------------------
    getRomBuffer(files) {
        const buffer = new Buffer(Generator.getRequiredRomSize(files));
    ...
    getPaddedRomBuffer(buffer, size) {
    ...
    getRequiredRomSize(files) {
        return files.map(Generator.getRequiredFileSize).sort((a, b) => {
            return b - a;

        })[0] || 0x8000; // Minimum ROM size is 32kbyte
    },

    getRequiredFileSize(file) {

        let v = file.sections.filter((s) => {
            return s.segment === 'ROM0' || s.segment === 'ROMX';

        }).map((s) => {
            return Math.floor(s.baseOffset / 0x4000);
    ...
    parseRom(buffer) {
        ...
        const rom = Generator.getRomInfo(buffer),
            g = Generator;

        // Validate logo
        for(let i = 0; i < g.NINTENDO_LOGO.length; i++) {
            if (g.NINTENDO_LOGO[i] !== rom.logo[i]) {
                rom.warnings.push(Generator.Warnings.INVALID_LOGO_DATA);
                Generator.applyLogo(buffer);
                break;
            }
        }
        ...
        if (g.COUNTRY_CODES.indexOf(rom.countryCode) === -1) {
            rom.errors.push(Generator.Errors.INVALID_COUNTRY_CODE);
            return rom;
        }
    ...
    applyLogo(buffer) {
        for(let i = 0; i < Generator.NINTENDO_LOGO.length; i++) {
            buffer[i + 0x104] = Generator.NINTENDO_LOGO[i];
        }
    },

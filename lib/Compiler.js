...
Compiler.prototype = {
    ...
    generate: function() {
        ...
        if (rom.rom.size > 32768) {
            this.log(null, 'ROM: ' + rom.rom.size + ' bytes ( ' + (rom.rom.size - 32768) + ' bytes in ' + rom.rom.banks + ' bank(s) )');

        } else {
            this.log(null, 'ROM: ' + rom.rom.size + ' bytes ( internal ROM only )');
        }

        if (rom.ram.size > 8192) {
            this.log(null, 'RAM: ' + rom.ram.size + ' bytes ( ' + (rom.ram.size - 8192) + ' bytes in ' + rom.ram.banks + ' bank(s) )');

        } else if (rom.ram.size > 0) {
            this.log(null, 'RAM: ' + rom.ram.size + ' bytes ( internal RAM only )');

        } else {
            this.log(null, 'RAM: None');
        }
        ...
        if (!this.silent) {
            if (file) {
                process.stderr.write(
                    '\n'
                    + file.getLineSource(error.index, error.name).source
                    + '\n\n'
                    + (error.name + ': ').red
                    + error.message
    ...
    warning: function(file, index, message) {
        if (!this.silent) {
            if (file) {
                process.stdout.write(file.getPath().yellow + ' ' + message.grey + '\n');

...
Compiler.infoFromSource = function(source) {
    ...
    const c = new Compiler(false, false),
        file = new SourceFile(
            c,
            null,
    ...
    file.instructions.forEach((i) => {
        opCycles += i.cycles;
        opBytes += i.size;
    });

    file.dataBlocks.forEach((d) => {
        dataBytes += d.size;
    });
...
Compiler.prototype = {
    ...
    generate() {
        ...
        if (rom.errors.length) {
            rom.errors.forEach((error) => {

                if (error === Generator.Errors.INVALID_ROM_SIZE) {
                    this.error(null, 'Invalid ROM size specified in ROM header');

                } else if (error === Generator.Errors.INVALID_RAM_SIZE) {
                    this.error(null, 'Invalid RAM size specified in ROM header');

                } else if (error === Generator.Errors.INVALID_CARTRIDGE_TYPE) {
                    this.error(null, 'Invalid cartridge type size specified in ROM header');

                } else if (error === Generator.Errors.MAPPER_UNSUPPORTED_RAM_SIZE) {
                    this.error(null, 'Mapper set in ROM header does not support the RAM size specified in the ROM header');

                } else if (error === Generator.Errors.MAPPER_UNSUPPORTED_ROM_SIZE) {
                    this.error(null, 'Mapper set in ROM header does not support the ROM size specified in the ROM header');
                }

            });

        } else if (rom.warnings.length) {
            rom.warnings.forEach((warning) => {

                if (warning === Generator.Warnings.ROM_IS_PADDED) {
                    this.warning(null, 0, 'Generated ROM image is bigger than the ROM size that was specified in the ROM header');

                } else if (warning === Generator.Warnings.HEADER_SIZE_TOO_SMALL) {
                    this.warning(null, 0, 'ROM size in header is smaller then the minimum required size for the generated ROM (automatically extended)');

                } else if (warning === Generator.Warnings.INVALID_LOGO_DATA) {
                    this.warning(null, 0, 'ROM contains invalid logo data (automatically patched to contain the correct values)');
                }
    ...
    json() {
        return JSON.stringify(Linker.getAllSections(this.files).map((l) => {
            return l.toJSON();
    ...
    symbols() {
        ...
        return symbols.map((s) => {
            const name = s.parent ? s.parent.name + s.name : s.name;
    ...
    unused() {
        ...
        this.files.forEach((f) => {
            ...
            f.labels.filter((l) => {
                ...
                this.warning(f.file, f.index, `Unused label: ${  f.name}`);
            });

            f.variables.filter((v) => {
                return v.references === 0;

            }).map((v) => {
                this.warning(v.file, v.index, `Unused variable: ${  v.name}`);
            });
    ...
    mapping() {
        ...
        Linker.getAllSections(this.files).map((s) => {

            const id = `${s.segment  }_${  s.bank}`;
        ...
        const map = segmentList.map((segment) => {
            ...
            segment.usage.forEach((u, index) => {

                const next = segment.usage[index + 1];
            ...
            const last = segment.usage[segment.usage.length - 1];
    ...
    // Internals --------------------------------------------------------------
    includeFile(parent, file, section, index) {
        const sourceFile = new SourceFile(this, parent, file, section, index);
    ...
    parse(files) {
        ...
        files.forEach((file) => {
            this.includeFile(null, path.join(process.cwd(), file), 0, null, 0, 0);
        });
    ...
    link(verify) {
        ...
        Linker.link(this.files, verify, this.debug);

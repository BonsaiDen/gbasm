...
Compiler.prototype = {
    ...
    // Internals --------------------------------------------------------------
    include: function(parent, file, section, line, col) {
        var sourceFile = new SourceFile(this, parent, file, section, line, col);
        this.files.push(sourceFile);
        sourceFile.parse();
        return sourceFile;
    },

    resolve: function(sourceFile, value, sourceOffset, sourceLine, sourceCol, relativeOffset, stack) {

        // Keep track and error on circular references
        stack = stack || [];

        if (stack.indexOf(value) !== -1) {
            // TODO keep track of files on tokens in order to correctly resolve the
            // actual site of the eventual circular reference
            sourceFile.resolveError(
                'Circular reference detected while resolving value',
                '"'  + stack[0].value + '" points at itself via ' + (stack.length - 2) + ' levels of indirection',
            ...
            case 'EXPRESSION':
                ...
                var expression = value.value.map(function(item) {
                    switch(item.type) {
                        ...
                        default:
                            var resolved = that.resolve(sourceFile, item, sourceOffset, item.line, item.col, false, stack);
                ...
                if (resolved) {
                    ...
                    if (resolved.hasOwnProperty('value')) {
                        if (typeof resolved.value === 'object') {
                            return this.resolve(sourceFile, resolved.value, sourceOffset, sourceLine, sourceCol, relativeOffset, stack);
                        ...
                        if (typeof resolved.offset !== 'number') {
                            throw new TypeError('Resolved offset is not a number.');
                        }
                        if (relativeOffset) {
    ...
    warning: function(file, line, col, message) {
        file = file ? file.getPath(line, col, true).orange : '[Warning]';
        console.error(file.getPath().orange + ' ' + message.yellow);
    },
